...
if (!fs.existsSync(RESTOCK_FILE)) {
  fs.writeFileSync(RESTOCK_FILE, JSON.stringify([]));
}

function getRestockRequests(): any[] {
  try {
    if (fs.existsSync(RESTOCK_FILE)) {
...
function saveRestockRequests(requests: any[]) {
  try {
    fs.writeFileSync(RESTOCK_FILE, JSON.stringify(requests, null, 2));
  } catch (e) {
    ...
    if (status === 'approved' && request.status !== 'approved') {
      for (const item of request.items) {
        ...
        if (product) {
          ...
          try {
            const { data: newProd, error: npErr } = await supabaseAdmin
              .from('products')
              .insert({
