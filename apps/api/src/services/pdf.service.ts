...
const BLUE_NAVY  = rgb(0.06, 0.18, 0.45); // Deep navy for headers and titles
const BLUE_ACCENT= rgb(0.12, 0.35, 0.78); // Bright royal blue for lines & badges
const BLUE_BG    = rgb(0.95, 0.97, 1.0);  // Soft pastel blue for Sold By card
const BLUE_BORDER= rgb(0.85, 0.90, 0.98); // Soft blue border
const GREEN_ACCENT= rgb(0.10, 0.60, 0.25); // Green for Billed To & Paid
const GREEN_BG   = rgb(0.95, 0.99, 0.95); // Soft pastel green for Billed To
const GREEN_BORDER= rgb(0.85, 0.94, 0.86);
const GRAY_BG    = rgb(0.97, 0.97, 0.98);
const GRAY_BORDER= rgb(0.88, 0.89, 0.92);
const GRAY_TEXT  = rgb(0.38, 0.40, 0.45);
const BLACK      = rgb(0.08, 0.08, 0.10);
const WHITE      = rgb(1, 1, 1);
const YELLOW     = rgb(1, 0.90, 0.15);    // Gold/yellow for total amount

function fetchImageBytes(url: string): Promise<Uint8Array | null> {
...
export async function generateInvoicePdf(invoiceId: string): Promise<Buffer> {
  ...
  const safe = (s: any): string =>
    String(s ?? "").replace(/[\u20B9]/g, "Rs.").replace(/[\u2014\u2013]/g, "-")
      .replace(/[^\x00-\xFF]/g, "");

  const txt = (t: any, x: number, y: number, sz = 9, bold = false, col = BLACK) => {
    const s = safe(t);
    if (!s) return;
    page.drawText(s, { x, y, size: sz, font: bold ? boldFont : font, color: col });
  };

  const fillRect = (x: number, y: number, w: number, h: number, col: typeof BLACK) => {
    page.drawRectangle({ x, y, width: w, height: h, color: col });
  };

  const drawBorder = (x: number, y: number, w: number, h: number, borderCol: typeof BLACK, bgCol?: typeof BLACK) => {
    page.drawRectangle({
  ...
  const hline = (y: number, x1 = ML, x2 = W - MR, thick = 0.5, col = GRAY_BORDER) => {
  ...
  const logoCandidates = [
    ...
    path.join(__dirname, "../../..", "web", "public", "logo.jpg"),
    path.join(__dirname, "..", "..", "logo.jpg"),
    path.join(process.cwd(), "logo.jpg"),
    path.join(process.cwd(), "apps", "web", "public", "logo.jpg"),
  ];
  ...
  const badgeCol = isPaid ? GREEN_ACCENT : rgb(0.85, 0.45, 0.1);
  ...
  const th = (t: string, x: number) => txt(t, x, y - TH_H + 7, 8, true, WHITE);
  th("#",            C_NUM);
  th("PRODUCT",      C_NAME);
  th("HSN",          C_HSN);
  th("QTY",          C_QTY);
  ...
  for (let i = 0; i < items.length; i++) {
    if (y - ROW_H < 170) break; // Overflow protection
