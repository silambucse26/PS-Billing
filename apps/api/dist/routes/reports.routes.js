...
const masterCatalog_1 = require("../config/masterCatalog");
const shopStatusStore_1 = require("../config/shopStatusStore");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const router = (0, express_1.Router)();
// Helper to seed or update default 20 Franchise Standard Products for a shop (944 units / ₹3,00,000 value)
async function seedDefaultProducts(shopId, allocateStandardStock = false, resetStockToZero = false) {
    try {
        ...
        const categoryMap = {};
        const { data: existingCats } = await supabaseAdmin_1.supabaseAdmin
            .from('categories')
            .select('id, name');
        ...
        const { data: existingProds } = await supabaseAdmin_1.supabaseAdmin
            .from('products')
        ...
        for (const p of masterCatalog_1.MASTER_PRODUCTS) {
...
router.get('/me', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (enrichedProfile.shop_id) {
            ...
            shop = s;
            if (shop) {
                const shopStatus = (0, shopStatusStore_1.getShopStatus)(shop.id) || shop.status || (shop.is_approved === false ? 'pending' : 'approved');
...
router.get('/admin/shops', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const { data: shops, error: sErr } = await supabaseAdmin_1.supabaseAdmin.from('shops').select('*').order('name', { ascending: true });
        if (sErr)
            throw sErr;
        ...
        if (fErr)
            throw fErr;
        const { data: allProducts } = await supabaseAdmin_1.supabaseAdmin
            .from('products')
            .select('id, shop_id, current_stock, sale_price, purchase_price, reorder_level');
        const { data: allInvoices } = await supabaseAdmin_1.supabaseAdmin
            .from('invoices')
        ...
        const enrichedShops = (shops || []).map(shop => {
            ...
            const status = (0, shopStatusStore_1.getShopStatus)(shop.id) || shop.status || (shop.is_approved === false ? 'pending' : 'approved');
            return {
                ...shop,
...
router.put('/admin/shops/:shopId/approve', auth_1.requireAuth, async (req, res) => {
    try {
        const role = req.user?.role;
        if (role !== 'super_admin') {
            return res.status(403).json({ error: 'Access denied: super_admin role required' });
        ...
        (0, shopStatusStore_1.setShopStatus)(shopId, status);
...
router.post('/admin/shops/:shopId/deploy-standard-stock', auth_1.requireAuth, async (req, res) => {
    try {
        const role = req.user?.role;
        if (role !== 'super_admin') {
            return res.status(403).json({ error: 'Access denied: super_admin role required' });
...
router.put('/admin/shops/:shopId/products/:productId/stock', auth_1.requireAuth, async (req, res) => {
    try {
        const role = req.user?.role;
        if (role !== 'super_admin') {
            return res.status(403).json({ error: 'Access denied: super_admin role required' });
        ...
        const { data: prod, error: pErr } = await supabaseAdmin_1.supabaseAdmin
            .from('products')
            .select('*')
            .eq('id', productId)
            .eq('shop_id', shopId)
        ...
        const { data: updatedProduct, error: uErr } = await supabaseAdmin_1.supabaseAdmin
            .from('products')
            .update({
...
router.delete('/admin/shops/:shopId', auth_1.requireAuth, async (req, res) => {
    try {
        const role = req.user?.role;
        if (role !== 'super_admin') {
            return res.status(403).json({ error: 'Access denied: super_admin role required' });
        ...
        const { data: targetShop, error: sErr } = await supabaseAdmin_1.supabaseAdmin
            .from('shops')
        ...
        (0, shopStatusStore_1.deleteShopStatus)(shopId);
        // 2. Cascade: Invoices & Invoice Items
        const { data: shopInvoices } = await supabaseAdmin_1.supabaseAdmin
            .from('invoices')
            .select('id')
            .eq('shop_id', shopId);
        if (shopInvoices && shopInvoices.length > 0) {
            const invoiceIds = shopInvoices.map((inv) => inv.id);
        ...
        try {
            ...
            if (fs_1.default.existsSync(RESTOCK_FILE)) {
                ...
                const filtered = list.filter((r) => r.shopId !== shopId && r.shop_id !== shopId);
                fs_1.default.writeFileSync(RESTOCK_FILE, JSON.stringify(filtered, null, 2));
            }
        }
        catch (e) {
        ...
        try {
            const { data: linkedProfiles } = await supabaseAdmin_1.supabaseAdmin
                .from('profiles')
                .select('id, role')
                .eq('shop_id', shopId);
            if (linkedProfiles && linkedProfiles.length > 0) {
                for (const p of linkedProfiles) {
                    if (p.role !== 'super_admin') {
                        try {
                            await supabaseAdmin_1.supabaseAdmin.auth.admin.deleteUser(p.id);
                        }
                        catch (errAuth) {
        ...
        const { error: delShopErr } = await supabaseAdmin_1.supabaseAdmin
            .from('shops')
            .delete()
            .eq('id', shopId);
        if (delShopErr) {
        ...
        if (targetShop.franchise_id) {
            try {
                const { data: remainingShops } = await supabaseAdmin_1.supabaseAdmin
                    .from('shops')
                    .select('id')
...
router.post('/admin/sync-master-catalog', auth_1.requireAuth, async (req, res) => {
    try {
        const role = req.user?.role;
        if (role !== 'super_admin') {
            return res.status(403).json({ error: 'Access denied: super_admin role required' });
        ...
        if (shopId) {
            ...
            const { data: allShops } = await supabaseAdmin_1.supabaseAdmin.from('shops').select('id');
        ...
        for (const sid of targetShopIds) {
            ...
            const { data: existingProducts } = await supabaseAdmin_1.supabaseAdmin
                .from('products')
                .select('*')
                .eq('shop_id', sid);
            for (const master of masterCatalog_1.MASTER_PRODUCTS) {
                ...
                const match = existingProducts?.find((ep) => (ep.sku && master.sku && ep.sku.trim().toLowerCase() === master.sku.trim().toLowerCase()) ||
                ...
                if (match) {
                    ...
                    await supabaseAdmin_1.supabaseAdmin.from('products').update(updatePayload).eq('id', match.id);
                    totalUpdated++;
                }
                else {
                    // Insert new product with stock 0 (or overrideStock)
                    const newStock = overrideStock !== undefined ? overrideStock : 0;
                    await supabaseAdmin_1.supabaseAdmin.from('products').insert({
                        shop_id: sid,
                        name: master.name,
                        sku: master.sku,
...
router.post('/admin/bulk-upsert-products', auth_1.requireAuth, async (req, res) => {
    try {
        const role = req.user?.role;
        if (role !== 'super_admin') {
            return res.status(403).json({ error: 'Access denied: super_admin role required' });
        ...
        for (const p of products) {
            ...
            const match = existingProducts?.find((ep) => (p.sku && ep.sku && ep.sku.trim().toLowerCase() === p.sku.trim().toLowerCase()) ||
                (p.barcode && ep.barcode && ep.barcode.trim() === p.barcode.trim()) ||
                (p.name && ep.name && ep.name.trim().toLowerCase() === p.name.trim().toLowerCase()));
            if (match) {
                await supabaseAdmin_1.supabaseAdmin.from('products').update({
...
router.post('/register-shop', async (req, res) => {
    try {
        ...
        (0, shopStatusStore_1.setShopStatus)(shop.id, 'pending');
